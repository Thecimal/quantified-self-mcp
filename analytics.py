...
def find_correlations(series_a: list[Point], series_b: list[Point], lag_days: int = 0) -> dict:
    ...
    This function is deliberately silent about how much to trust the
    result — no bucketed read on "n" alone. That judgment belongs to
    qs_evidence.assess_correlation, which folds the paired-sample size
    into the evidence profile's sample dimension and produces a single
    claim decision. See CorrelationResult.claim.
    """
